...
def detect_ranges(L):
    ...
    # for the random list it's stuck in the 2nd while loop
        ...
        while (all(x in ls for x in new_list)):# and (j < len(ls)): # CHANGER 2NDE CONDITION
            ...
            j += 1
            #i += 1
        if (ls[-1] == new_list[-1] + 1) and (len(ls) != len(new_list)): # je rajoute une condition fictive
                #print("(" + str(new_list[0]) + "," + str(ls[-1] + 1), end = ")]")
            couple2 = tuple((new_list[0], ls[-1] + 1))
        ...
        while (not all(x in ls for x in new_list)):# and (j < len(ls)): ## CHANGER 2NDE CONDITION
            if len(new_list) == 2:
                ...
                #result.append(new_list[-1])
                    ...
                    result.append(new_list[0])
                if len(ls) != len(new_list): # je rajoute une condition fictive
...
def main():
    ...
    #L = [-2, 0, 1, 2, 3]  # works
    L = [1, 2, 4] # does not work # works with modified code
    #L = [4, 2, 0, -2, -4] # list index out of range
    #L = [-32, -22, -85, 76, -54, -49, 79, -17, -10, 86] # random list
